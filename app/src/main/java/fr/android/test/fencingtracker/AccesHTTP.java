...
public class AccesHTTP extends AsyncTask<String, Integer, Long> {
    ...
    public AsynResponse delegate=null; // gestion du retour asynchrone
...
public class AccesHTTP extends AsyncTask<String, Integer, Long> {
    ...
    @Override
    //Conexion en fond dans un thread séparé
    protected Long doInBackground(String... strings) {

        HttpClient cnxHttp = new DefaultHttpClient();
        HttpPost paramCnx = new HttpPost(strings[0]);
        try{
            //encoder les parametres
            paramCnx.setEntity(new UrlEncodedFormEntity(parametres));
            //connexion en envoie parametres, attente de reponse
            HttpResponse reponse = cnxHttp.execute(paramCnx);
            //transformation de la rpeonse
